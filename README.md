...
星远（GVY）正式官网；舰队正式名称和 Logo 锁定区使用“星远”，介绍、招募与对外口语使用“星远舰队”。项目是由语义化 HTML、CSS、原生 ES Modules 和 GSAP 组成的静态站；仓库还包含一个只在 macOS 本机运行的 React 相册发布器。蓝图站与维科洛站是独立项目，本仓库只提供同页导航入口，不拥有或部署它们。
