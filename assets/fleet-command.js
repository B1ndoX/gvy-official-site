...
function updateIntroProgress() {
  ...
  const titleExit = 1 - smoothRange(rawProgress, 1.42, 2.08);
  const titleLabel = smoothRange(progress, 0.03, 0.18) * titleExit;
  const titleMain = smoothRange(progress, 0.16, 0.42) * titleExit;
  const titleSub = smoothRange(progress, 0.36, 0.58) * titleExit;
  const titleMark = smoothRange(progress, 0.48, 0.68) * titleExit;
  const title = smoothRange(progress, 0.08, 0.7) * titleExit;
  const motto = smoothRange(progress, 0.62, 0.88) * titleExit;
  const hud = smoothRange(progress, 0.68, 0.86);
  const actions = smoothRange(progress, 0.78, 0.94);
  const exit = smoothRange(rawProgress, 1.42, 2.08);
  const navReveal = smoothRange(progress, 0.66, 0.82);
  const scrollCue = 1 - smoothRange(progress, 0.035, 0.18);
  const videoFade = smoothRange(rawProgress, 0.86, 2.02);
  const blackout = smoothRange(rawProgress, 1.02, 2.08) * 0.34;
