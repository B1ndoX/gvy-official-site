...
- EdgeOne 正式静态构建固定使用平台官方推荐、预装的 Node `22.11.0`，只执行 `npm run verify:edgeone`；这是部署平台兼容性边界，不代表开发主版本。该命令仍检查全部媒体文件是否存在且体积合规，只跳过 EdgeOne 容器因未预装 `ffmpeg` 而无法执行的编解码、分辨率和时长元数据检查。
...
- EdgeOne 自身还会运行 `npm run verify:edgeone`；验证失败时不得生成新生产部署。GitHub Actions 继续使用带 `ffmpeg` 的 `npm run verify:site` 完成全部媒体元数据审计，不得把 `GVY_SKIP_MEDIA_METADATA=1` 用到本地完整验证或 GitHub CI。
